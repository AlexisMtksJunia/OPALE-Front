...
function makePromotions(cycleName, years) {
    // année = 1..years
...
export default function Promotions() {
    ...
    const renameCycle = (cycleId, name) => {
        setCycles(prev => {
            ...
            const oldName = cycle.name
            const renamed = prev.map(c => {
                if (c.id !== cycleId) return c
                // on renomme aussi les promotions existantes pour garder “Nom X”
                const promotions = c.promotions.map((p, idx) => ({
                    ...p,
                    label: `${name} ${idx + 1}`,
                }))
                return { ...c, name, promotions }
            })
            console.log('[CYCLES] rename', { cycleId, oldName, name })
            return renamed
        })
    }
    ...
    const removePromotion = (cycleId, promoId) => {
        setCycles(prev => {
            ...
            console.log('[PROMO] remove', { cycleId, promoId })
            return next
        })
    }

    const editPromotion = (cycleId, promoId) => {
        // Ici on ouvrirait un panneau/boîte plus tard (nb étudiants, nb classes, ...)
    ...
    return (
        <div className="promos">
            <div className="promos-header">
                ...
                <p className="page-sub">Cycles (diplômes) et leurs promotions annuelles.</p>
                <button className="btn-primary" onClick={addCycle}>+ Ajouter un cycle</button>
            </div>
